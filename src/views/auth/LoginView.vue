    ...
    <div class="auth-card-wrapper">
      <div class="auth-card">
        ...
        <div class="auth-title-block">
          <h4 class="auth-title">{{ t.loginTitle }}</h4>
        ...
        <form @submit.prevent="submit" class="auth-form">
          ...
          <div class="forgot-row">
            <router-link to="/admin/auth/forgot-password" class="forgot-link">
              {{ t.forgotPassword }}
            </router-link>
          </div>
          ...
          <p class="auth-alt-link">
            {{ t.noAccount }}
            <router-link to="/admin/auth/register" class="alt-link">{{ t.signUpHere }}</router-link>
          </p>
  ...
  setup() {
    ...
    const submit = async () => {
      ...
      try {
        ...
        if (serverMsg.toLowerCase().includes('not found') || serverMsg.toLowerCase().includes('user')) {
          errorMessage.value = t.value.loginErrorEmail
        } else if (serverMsg.toLowerCase().includes('password') || error.response?.status === 401) {
          errorMessage.value = t.value.loginErrorPassword
        } else {
          errorMessage.value = t.value.loginError
        }
      } finally {
        loading.value = false
      }
    ...
    return { email, password, loading, submit, t, currentLocale, setLanguage, showPassword, errorMessage }
  },
  data() {
...
/* ─── Forgot ───────────────────────────────────────── */
.forgot-row { text-align: right; margin-top: -0.4rem; }
.rtl-page .forgot-row { text-align: left; }
...
.forgot-link:hover { color: #0A1870; text-decoration: underline; }

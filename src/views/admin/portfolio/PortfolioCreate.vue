<template>
  <div>
    <div class="page-header mt-3" :class="{ rtl: isRtl }">
      <router-link to="/admin/portfolio" class="back-btn"><i class="bi bi-arrow-right"></i></router-link>
      <h1 class="page-title"><i class="bi bi-plus-circle"></i> {{ t('newItem') || 'عمل جديد' }}</h1>
    </div>
    <PortfolioForm
      :categories="categories" :errors="errors" :loading="loading" :is-edit="false"
      @submit="handleSubmit" @cancel="$router.push('admin.portfolio.index')"
    />
  </div>
</template>
